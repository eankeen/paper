...
export class CategoryColumn {
  ...
  render() {
		...
		const shouldBeSelected = (index: number) =>{
			...
			return this.selectedCard.y === index && this.selectedCard.x === this.columnIndex
		}
		...
		<article class="category-column">
			{
				(this.cards || defaultCards).map(({name, status }, rowIndex) => {
					return <category-card key={rowIndex} name={name} status={status}
						isSelected={shouldBeSelected(rowIndex)}></category-card>
					})
			}
      </article>
    )
